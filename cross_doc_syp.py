...
def run_SyPRIS(cof_list, scaff_list, num_cpu):
    ...
    for cof_start in cof_list:
        cof_start = cof_start.rstrip('\r\n')
        cof = 'symmed_cofs/' + cof_start
...
def syp_pkg_tester(cof_file, scaff_file):
    ...
    cof.generate_inverse_rot_ensemble(std_periodicity=4, lasa='CB', my_rotlib='./rotlib_cont.txt')
    ...
    if cof.match_data_set:
        ...
        for k,v in cof.match_cofactors.iteritems():
            ...
            if v.clash_check(test_scaff=scaffy, refine=True) == False:
                out_line = [cof_file.split('_cof_symmed2.pdb')[0][12:], k, scaff_file]
                sep_name = k.split('_')
                res_to_check = []
                if len(sep_name) == 4:
        ...
        if out_lines:
            with open('sub_%s_cross_trimer.csv' % out_lines[0][0], 'a') as csvfile:
...
def main():
    ...
    cof_file_list = args[2]
    jobs = args[3]

    with open(scaff_file_list, 'r') as mf:
        scaff_list = mf.readlines()
    with open(cof_file_list, 'r') as mc:
        cof_list = mc.readlines()
    
    ffm = run_SyPRIS( cof_list, scaff_list, jobs )
