...
class Transform(object):    
    ...
    def __init__(self, pose):
        if type(pose) == list or type(pose) == tuple:    
            ...
            if type(pose[0]) == str:
                self.xyz_total,\
                self.geo_center_mags = pdb_to_xyz(pose)
                e1 = time.clock()
    ...
    def rotate_object(self, atom1, atom2, theta):
        ...
        for i, atom in enumerate(self.xyz_total):
            if atom == [0.0, 0.0, 0.0]:
                ...
                Rxyz = ((Rxyz[0] * self.geo_center_mags[i]),\
                        (Rxyz[1] * self.geo_center_mags[i]),\
                        (Rxyz[2] * self.geo_center_mags[i]))
                #print 'Rxyz  = ' + str(Rxyz)
    ...
    def translate(self, final, initial = []):
        ...
        self.xyz_total = [ [x[0] +translation[0], x[1] +translation[1], x[2] + translation[2]] for x in self.xyz_total]
