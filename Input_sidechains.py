...
def syp_pkg_tester(cof_file, scaff_file):
    ...
    cof.generate_inverse_rot_ensemble(std_periodicity=1, lasa='CB', my_rotlib='./rotlib.txt')
    ...
    if cof.match_data_set:
        ...
        for k,v in cof.match_cofactors.iteritems():
            ...
            if v.clash_check(test_scaff=scaffy, include_resnum=list(included_res)) == False:
                out_line = [cof_file.split('_cof.pdb')[0][5:], k, scaff_file]
                sep_name = k.split('_')
                res_to_check = []
                if len(sep_name) == 4:
                ...
                for k2 in res_to_check:
                    ...
                    #v.write_to_file('%s_%s_rmsa_%s_rmsd_%s_score_%s.pdb' % ( scaff_file[:-4], k, round(rmsa,10), round(rmsd, 10), round(score, 10) ) )
                #cof.scaffold.write_to_file('%s_syp_centered.pdb' % scaff_file[:-4])
                out_lines.append(out_line)
        if out_lines:
            with open('%s.csv2' % out_lines[0][0], 'a') as csvfile:
